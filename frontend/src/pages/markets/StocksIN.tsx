...
import { stocksIN } from '../../data/markets/stocksIn';
import { addToWatchlist, isInWatchlist } from '../../lib/watchlist';
export function StocksIN() {
  const [search, setSearch] = useState('');
  const [sectorFilter, setSectorFilter] = useState('all');
  const [sortBy, setSortBy] = useState('mcap');
  const sectors = Array.from(new Set(stocksIN.map(s => s.sector)));
  const filteredStocks = stocksIN.filter(stock => {
    ...
    if (sortBy === 'mcap') return b.mcapCr - a.mcapCr;
  ...
  return <PageContainer title="Indian Stocks (NSE/BSE)" description="Track top Indian stocks">
      <div className="mb-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredStocks.map(stock => <StockCard key={stock.symbol} stock={stock} type="IN" onAddToWatchlist={() => handleAddToWatchlist(stock)} onClick={() => {}} isInWatchlist={isInWatchlist(stock.symbol, 'stock-in')} />)}
      </div>
    </PageContainer>;
}